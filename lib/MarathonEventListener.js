...
function MarathonEventListener (options) {
    ...
    // Master discovery
    self.options.marathonUrl = options.marathonUrl || "leader.mesos";
    self.options.marathonPort = parseInt(options.marathonPort) || 8080;
    self.options.protocol = options.protocol || "http";
    self.options.marathonConnectionTimeout = parseInt(options.timeout) || 10000;
    self.options.slackChannel = options.slackChannel || process.env.SLACK_CHANNEL || "#marathon";
    self.options.slackWebHook = options.slackWebHook || process.env.SLACK_WEBHOOK;
...
MarathonEventListener.prototype.subscribe = function () {
    ...
    var self = this,
        url = self.options.protocol + "://" + self.options.marathonUrl + ":" + self.options.marathonPort+"/v2/events";
...
MarathonEventListener.prototype.handleEvent = function (event) {
    ...
    function parseEvents (event) {
        ...
        var message = {
            ...
            "icon_url": "http://i.imgur.com/4ripYOu.png",
            "mrkdwn": true
        };
